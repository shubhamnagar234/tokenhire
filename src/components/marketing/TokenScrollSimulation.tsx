...
export function TokenScrollSimulation() {
  ...
  useEffect(() => {
    ...
    mm.add("(max-width: 767px)", () => {
      // Reset inline styles that desktop might have applied
      gsap.set([text1Ref.current, text2Ref.current, text3Ref.current, ".editor-window"], { clearProps: "all" });
      ...
      // Simple animation for mobile: just type the code, drain tokens, show popup. 
      // No text fading since they are stacked and all visible.
      gsap.set([aiPopupRef.current, scorePopupRef.current, ".code-line-final"], { opacity: 0, y: 20 });
      gsap.set(".code-line", { opacity: 0, x: -10 });
      ...
      tl.to(tokenBarRef.current, { width: `${tPct}%`, backgroundColor: tokenColor, duration: 2 }, "<");

      tl.to(tokenCountRef.current, {
        innerText: tRem,
        duration: 2,
        snap: { innerText: 1 },
        onUpdate: function () {
      ...
      tl.to(".editor-window", { opacity: 0.2, scale: 0.95, duration: 1 })
        .to(aiPopupRef.current, { opacity: 0, duration: 0.5 }, "<")
        .to(scorePopupRef.current, { opacity: 1, y: 0, scale: 1, duration: 1 }, "<");
    });
  ...
  return (
    <div
      ref={containerRef}
      className="w-full bg-background flex items-center justify-center overflow-hidden border-t border-b border-border relative py-16 md:py-0 md:h-screen"
    >
      ...
      <div className="max-w-6xl w-full px-6 grid md:grid-cols-2 gap-16 items-center relative z-10">
        ...
        <div className="relative md:h-[300px] flex flex-col md:block items-start gap-10 md:gap-0">
          <div
            ref={text1Ref}
            className="md:absolute md:inset-0 flex flex-col justify-center space-y-4"
          >
